...
Dans la continuité des traveaux menés par Alexandre Duplan, nous avons
mis en oeuvre des procédures de selections de variables afin de
construire des modèles de prédictions de phénotypes à partir des données
génomiques.

Notre objectif est d'améliorer les performances de prédictions

Notre étude se décompose en deux parties ou nous explorerons différentes
procédures de sélection de variables. Dans un premier temps nous
présenterons les résultats obtenus par des approches linéaires de
sélections de variables, à savoir le MLM et FarmCPU. Dans un second
temps nous présenterons une procédure de sélection basée sur les scores
d'importances obtenus via Random Forest.
...
Nous émettons l'hypothèse que le scoring
\href{Plot/CIRC2009_RR_Brut}{dff} RF aboutit à une sélection de SNPs
plus restreinte mais plus dense en information. En isolant les
...
\subsection{Correction de la structure de population et de
...
\[y = P\beta + u + \epsilon\] \[P\beta\]
\[\hat{\epsilon} = y - (P\hat{\beta} + \hat{u})\] MLMM
...
Où :\(\hat{d}_j\) : L'effet estimé du SNP (la ``pente'' de la relation
entre le génotype et le phénotype).\(SE(\hat{d}_j)\) : L'erreur standard
de cet effet, calculée par :

\[SE(\hat{d}_j) = \sqrt{\frac{\sigma_e^2}{(n-k) \cdot Var(S_j)}}\] (avec
\(\sigma_e^2\) la variance résiduelle, \(n-k\) les degrés de liberté et
\(Var(S_j)\) la variance du génotype).

La P-value est la probabilité que la valeur absolue de \(T\) (variable
aléatoire suivant une loi de Student) soit supérieure à la valeur
\(|t|\) observée, sous l'hypothèse nulle (\(H_0 : d_j = 0\)).
...
Où :\(f(x, \nu)\) est la densité de probabilité de la loi de
Student.\(\nu = n - k\) représente les degrés de liberté (nombre
d'individus moins nombre de paramètres estimés comme les PC et les
pseudo-QTN).

modèle a effet aléatoires :

\[y = \mu + u + e\]
\[u \sim \mathcal{N}(0, \mathbf{K}_{pseudo}\sigma_a^2)\] avec
\[\sigma_a^2\] la variance additive

\[Var(u) = 2\mathbf{K}_{pseudo}\sigma_a^2\]

Où :\(u\) : L'effet génétique aléatoire total.\(\mathbf{K}_{pseudo}\) :
La matrice de parenté (Kinship) calculée uniquement à partir des
pseudo-QTN sélectionnés par l'algorithme SUPER.\(\sigma_a^2\) : La
variance génétique additive estimée par REML (Restricted Maximum
Likelihood).
...
\tightlist
\item
  Étape de Déflation (Modèle Mixte)
\end{enumerate}
...
2.2. Étape de Modélisation (Random Forest) On ajuste un ensemble de
\(B\) arbres de décision

\[\hat{\epsilon}_{i,m} = \frac{1}{B} \sum_{b=1}^{B} T_b(x_{i, \mathcal{X}_m})\]
\(T_b\) représente un arbre individuel \(x_i\) représente les valeurs
des SNPs de l'individu \(i\) pour les colonnes sélectionnées
...
\tightlist
\item
  Score d'importance pour un SNP i :
\end{enumerate}

\[Score_j =\Delta I(S_j) = \sum_{n} [w_n \cdot \text{Var}(n) - (w_L \cdot \text{Var}(n_L) + w_R \cdot \text{Var}(n_R))]\]
\(n\) (Le nœud parent) : C'est le groupe d'individus (peupliers) juste
avant d'être divisé par le SNP \(j\).\(n_L\) et \(n_R\) (\(L\) pour
Left, \(R\) pour Right) : Ce sont les deux sous-groupes créés après la
division (les ``nœuds fils'')

\(w_n, w_L, w_R\) (Les poids) : Ils correspondent à la proportion
d'individus présents dans chaque nœud par rapport au nombre total
d'individus dans l'échantillon
(\(N\)).\(w_n = \frac{N_{individus\_dans\_n}}{N_{total}}\)\(w_L = \frac{N_{individus\_dans\_n_L}}{N_{total}}\)\(w_R = \frac{N_{individus\_dans\_n_R}}{N_{total}}\)

et on utilise la version ``corrected'' (basée sur l'approche de Nembrini
et al.), le score final pour le SNP \(j\) est :

\[Score_j = \Delta I(X_j)_{obs} - \Delta I(X_j)_{perm}\]

4 . Agrégation et Ranking Le score final de ton fichier Parquet
(\(S_{final}\)) pour un SNP \(j\) est la moyenne des scores obtenus sur
toutes les tâches (ici 870 pour selectionner chaque variable en moyenne
20 fois) :
...
Pour chaque méthode, on définit d'abord le sous-ensemble \(\mathcal{S}\)
des \(K\) marqueurs les plus importants :
\[\mathcal{S} = \{ j \mid \text{rank}(\text{score}_{j}) \le K \}\]

Ce sous-ensemble permet de construire la matrice de design restreinte
par concaténation des colonnes de génotypes sélectionnées :
