...
Dans la continuité des traveaux menés par Alexandre Duplan, nous avons mis en oeuvre des procédures de selections de variables afin de construire des modèles de prédictions de phénotypes à partir des données génomiques. 

Notre objectif est d'améliorer les performances de prédictions 

Notre étude se décompose en deux parties ou nous explorerons différentes procédures de sélection de variables. Dans un premier temps nous présenterons les résultats obtenus par des approches linéaires de sélections de variables, à savoir le MLM et FarmCPU. Dans un second temps nous présenterons une procédure de sélection basée sur les scores d'importances obtenus via Random Forest. 
...
++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
>>>>>>> d78de19a8992672cab89534ad47a56a91b7c3633
# Caractérisation des ressources génomiques et phénotypiques

La prédiction de phénotypes complexes chez Populus nigra repose sur l’exploitation conjointe de données génomiques à très haute dimension (≈217 000 SNPs) et de mesures phénotypiques couvrant un large spectre de traits (croissance, phénologie, morphologie, biochimie). Cependant, un obstacle majeur limite l’interprétation et la performance des modèles : la structure de population fortement hiérarchisée de l’espèce.
Comme le montrent les analyses préliminaires et les travaux antérieurs (Duplan, 2025), la variance génétique est largement dominée par l’origine géographique des individus, ce qui impose une caractérisation fine des composantes démographiques avant toute modélisation prédictive.

## Etude de la structure des populations de Populus nigra

L'échantillonnage de l'étude repose sur 199 individus de Populus nigra, issus de 11 populations naturelles distribuées le long de grands bassins versants européens.

![Repartition geographique des populations](Plot/Screenshot 2026-02-02 183236.png)


Le peuplier noir étant une espèce strictement ripisylve, son écologie est intrinsèquement liée à la dynamique des cours d'eau. Cette dépendance aux réseaux hydrographiques limite drastiquement les flux de gènes (pollen et graines) entre les différentes vallées.

En raison de cet isolement géographique, chaque bassin versant fonctionne comme une unité évolutive distincte, ou "île génétique", où la dérive génétique et l'adaptation locale s'exercent de manière isolée. Il en résulte une forte différenciation génétique inter-population, que l'on observe très clairement sur l'Analyse en Composantes Principales (ACP). 

![ACP sur SNPs](Plot/Screenshot 2026-02-02 182541.png)
...
Ces résultats confirment que la structure de population (P) constitue la première source de variance génétique. Cela signifie que, la proximité génétique entre deux arbres reflète d'abord leur proximité géographique.


## Structure fine : La parenté cryptique (Matrice K)

« Au-delà de la structuration globale entre les bassins versants, il existe une structure plus subtile à l'échelle locale. Pour la capturer, nous utilisons une matrice de parenté (K), ou Kinship matrix. Cette matrice permet de modéliser :

Les relations d'apparentement étroites : Elle identifie les individus partageant des ancêtres récents au sein d'une même vallée.

La covariance génétique résiduelle : Elle capture les corrélations que les premières composantes principales (PCs) de la population ne parviennent pas à absorber.

Le bruit de fond polygénique : Elle intègre l'effet combiné de l'ensemble des marqueurs du génome ayant des effets mineurs sur le trait.

L'intégration conjointe de cette double structuration (P + K) est impérative. Sans elle, nous ferions face à un biais de confusion majeur : un SNP pourrait être déclaré statistiquement significatif alors qu'il n'est qu'un simple marqueur de l'origine géographique ou familiale de l'arbre, sans lien biologique fonctionnel avec le phénotype. »


## Déséquilibre de Liaison (LD) et Architecture Génétique

Enfin, l’efficacité de la détection des QTLs repose sur le Déséquilibre de Liaison (LD). Le génome du peuplier présente des blocs d’haplotypes, au sein desquels les SNPs sont fortement corrélés (LD). 
Un haplotype est un ensemble de variations génétiques héritées ensemble en raison de leur proximité sur le chromosome. Il peut inclure des gènes ou des régions du génome qui influencent des traits quantitatifs, ces régions étant appelées QTLs (Quantitative Trait Loci).

![LD](Plot/Screenshot 2026-02-02 183452.png)

![QTL](Plot/Screenshot 2026-02-02 183543.png)

Cette architecture génétique est une lame à double tranchant :

Un levier de détection : Elle facilite la découverte de QTLs. En effet, grâce au LD, un SNP mesuré peut "tagger" (ou servir de balise à) un variant causal non observé, permettant ainsi une détection indirecte: c'est l'effet proxy.

Un défi d'interprétation : Elle induit une forte redondance informationnelle. Plusieurs SNPs colinéaires portent la même information, ce qui complexifie l'identification du véritable variant fonctionnel.

La combinaison du LD et de la structure de population peut générer des signaux d'association erronés (faux positifs). C’est pourquoi nous devons appliquer des méthodes d'élagage (LD pruning) et effectuer une étude d'association GWAS (Genome-Wide Association Study) via des modèles mixtes, afin d'isoler le signal biologique réel de la simple corrélation statistique.


## Génération de phénotypes de contrôle (traits nuls)


Afin d’évaluer la robustesse des méthodes de sélection de SNPs et de prédiction génomique, nous avons généré plusieurs phénotypes simulés.
Ces traits nuls permettent :

de tester la sensibilité des modèles à la structure de population,
d’évaluer leur capacité à ne pas détecter de faux signaux,
de comparer les performances sur des phénotypes plus ou moins structurés.

Les simulations reposent sur les données de localisation (Population, Latitude, Longitude, Altitude) et sur l’alignement strict entre matrices génomiques et phénotypiques.

### Phénotype structuré par la population
Ce premier phénotype simule un trait entièrement déterminé par l’origine géographique, reproduisant un cas où la structure de population domine la variance phénotypique (cas fréquent chez Populus nigra). Ce trait est utile pour tester la capacité des modèles à ne pas confondre structure et signal biologique.

### Phénotype structuré par la géographie
Ce phénotype dépend d’un gradient environnemental (latitude, longitude, altitude), simulant un trait adaptatif. Il permet d’évaluer la sensibilité des modèles aux gradients spatiaux continus, souvent confondus avec des signaux génétiques.

### Phénotype mixte (population + géographie)
Ce dernier phénotype combine effets démographiques et gradients géographiques, reproduisant un cas réaliste où :
une partie du trait est liée à l’histoire évolutive,
une autre à l’adaptation locale.
Ce phénotype est particulièrement utile pour tester la capacité des modèles à démêler structure et signal fonctionnel, un enjeu central de l’étude.


## Correction de la structure de population et de l'apparentement


Chez Populus nigra, la forte structuration démographique entraîne des corrélations non causales entre génotypes et phénotypes : un SNP peut sembler associé à un trait simplement parce qu’il caractérise une population, et non en raison d’un effet biologique réel (Price et al., 2006). Ce phénomène provoque des faux positifs en GWAS et peut surestimer les performances prédictives.

Dans une population fortement structurée comme celle du peuplier noir, trois sources d’information génétique sont entremêlées :

Structure de population ($P$) : différences génétiques massives entre bassins versants

Parenté ($K$) : ressemblance génétique entre individus apparentés

Effets des QTLs : signal biologique réellement lié au phénotype

Dans les données brutes, ces composantes sont colinéaires, et un modèle naïf peut obtenir un R² élevé simplement en capturant $P$ et $K$, sans jamais identifier les variants causaux. C’est ce qui explique le paradoxe observé dans la thèse de Duplan : une sélection aléatoire de SNPs prédit aussi bien qu’une sélection ciblée, car les deux reflètent essentiellement la structure démographique.

Pour isoler le signal fonctionnel, il est donc indispensable de déflater le phénotype des composantes $P$ et $K$ avant toute sélection de SNPs. Cette correction est réalisée grâce au modèle linéaire mixte $Q+K$ (Price et al., 2006 et Yu et al. 2006), référence actuelle pour contrôler la stratification en génétique des populations.
...
Le premier modèle utilisé est un LMM classique (Q+K), qui corrige simultanément :

la structure de population via $P\beta$
la parenté via $u$
...
$P\beta$ : effets fixes (PCs, populations, covariables)
$S_i$: génotype du SNP testé $i$
$\alpha_i$: : effet du $i^e$ SNP testé
$u\sim{N(0,K\sigma_{g}^2)}$ : effet aléatoire de parenté
$\epsilon\sim{N(0,I\sigma_{e}^2)}$ : bruit résiduel


Une fois le modèle ajusté, on extrait les résidus déflatés :

$$\hat{\epsilon} = y - (P\hat{\beta} + \hat{u})$$

Ces résidus représentent la partie du phénotype non expliquée par la structure ni par la parenté, et servent ensuite :

à la sélection de SNPs
à la prédiction génomique
à l’analyse épistatique


## Modèle Linéaire Mixte Multi-locus (MLMM) : ajout de cofacteurs

Le MLM simple teste chaque SNP indépendamment, ce qui pose problème lorsque plusieurs QTLs influencent le trait. Le MLMM améliore ce modèle en ajoutant progressivement des cofacteurs (SNPs significatifs) dans le modèle :

$$y = P\beta + \underbrace{\sum_{m=1}^{k} S_m\alpha_m}_{\text{Co-facteurs}} + \underbrace{S_{i}\alpha_{i}}_{\text{SNP testé}} + u + \epsilon$$
avec : 
$S_m$ : cofacteurs déjà sélectionnés
$S_i$ : SNP actuellement testé
$k$ : nombre de cofacteurs retenus

Le MLMM se distingue par sa capacité à corriger les effets de QTLs majeurs en les intégrant directement comme covariables dans le modèle, ce qui permet de réduire les faux positifs liés à la structure résiduelle et d’améliorer la détection de QTLs secondaires. Il reste fondé sur le principe $Q+K$, en contrôlant simultanément la structure de population et la parenté. Cette approche est particulièrement adaptée lorsque plusieurs QTLs exercent des effets modérés, que certains signaux sont masqués par un QTL majeur, et dans les contextes où la structure de population est forte.

## FarmCPU : Fixed and Random Model Circulating Probability Unification

Le FarmCPU est une évolution du MLMM, conçue pour :

réduire les faux positifs
augmenter la puissance
éviter la sur-correction par $K$
accélérer les calculs

Le FarmCPU sépare explicitement :

Modèle fixe (FEM), qui teste chaque SNP en présence de pseudo-QTNs :
 
$$y = P\beta + \underbrace{\sum_{m=1}^{k} S_m\alpha_m}_{\text{pseudo-QTNs}} + \underbrace{S_{i}\alpha_{i}}_{\text{SNP testé}} + \epsilon$$

Modèle aléatoire (REM), qui met à jour la liste des pseudo-QTNs en utilisant $K$.

$$y = \mu + u + \epsilon$$
avec:
$\mu$ : effet fixe (intercepte) 
$u$ : effets aléatoires $u \sim N(0, K_{pseudo}\sigma^2_g)$ 
$K_{pseudo}$ : kinship construite uniquement à partir des pseudo-QTN
$\sigma^2_g$: variance génétique additive 

Les pseudo-QTNs jouent le même rôle que les cofacteurs du MLMM, mais sont mis à jour itérativement.
Statistique de test :

$$W = \frac{\hat{\alpha}^2}{\text{Var}(\hat{\alpha})} \sim \chi^2$$

calcul de la P-Value par t-test : 

$$t = \frac{\hat{d}_j}{SE(\hat{d}_j)}$$

Où :$\hat{d}_j$ : L'effet estimé du SNP (la "pente" de la relation entre le génotype et le phénotype).$SE(\hat{d}_j)$ : L'erreur standard de cet effet, calculée par :

$$SE(\hat{d}_j) = \sqrt{\frac{\sigma_e^2}{(n-k) \cdot Var(S_j)}}$$
(avec $\sigma_e^2$ la variance résiduelle, $n-k$ les degrés de liberté et $Var(S_j)$ la variance du génotype).

La P-value est la probabilité que la valeur absolue de $T$ (variable aléatoire suivant une loi de Student) soit supérieure à la valeur $|t|$ observée, sous l'hypothèse nulle ($H_0 : d_j = 0$).

$$P\text{-value} = 2 \cdot P(T > |t|)$$

Elle se calcule en intégrant la fonction de densité de la loi de Student :

$$P\text{-value} = 2 \int_{|t|}^{+\infty} f(x, \nu) \, dx$$

Où :$f(x, \nu)$ est la densité de probabilité de la loi de Student.$\nu = n - k$ représente les degrés de liberté (nombre d'individus moins nombre de paramètres estimés comme les PC et les pseudo-QTN).


modèle a effet aléatoires : 

$$y = \mu + u + e$$
$$u \sim \mathcal{N}(0, \mathbf{K}_{pseudo}\sigma_a^2)$$
avec $$\sigma_a^2$$ la variance additive


$$Var(u) = 2\mathbf{K}_{pseudo}\sigma_a^2$$

Où :$u$ : L'effet génétique aléatoire total.$\mathbf{K}_{pseudo}$ : La matrice de parenté (Kinship) calculée uniquement à partir des pseudo-QTN sélectionnés par l'algorithme SUPER.$\sigma_a^2$ : La variance génétique additive estimée par REML (Restricted Maximum Likelihood).




## Construction de la matrice de parenté (K)
...
# Analyse comparative des méthodes de scoring des SNPs : Moïse 
