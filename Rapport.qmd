---
title: "Prédiction de phénotypes chez Populus nigra par différentes approches de sélection de variables génomiques"
...
diag_matrix <- data.frame(
  Comparaison = c("FarmCPU vs MLM", "FarmCPU vs RF", "MLM vs RF"),
  Angle_Insertion = c("0.63", "0.74", "0.17"),
  Circonference_2009 = c("0.05", "0.31", "0.28"),
  Phenotype_Aleatoire = c("0.02", "0.28", "0.27"),
  Interpretation_Globale = c(
    "Divergence conditionnelle au succès FarmCPU",
    "Faible consensus",
    "Faible consensus"
  )
)

# --- Rendu Kable (Correction de l'alignement) ---
kable(
  diag_matrix,
  caption = "Matrice des scores EMD-LD entre les signaux identifiés par différentes procédures de sélections (Top 5000 SNPs).",
  align = "lcccl", 
  booktabs = TRUE,
  # Utilisation de \n pour réduire la largeur des en-têtes
  col.names = c("Comparaison", "Angle insertion", "Circ 2009", "Phéno Aléatoire", "Interprétation")
) %>%
  kable_styling(
  ...
  # On fixe la largeur de la colonne interprétation pour forcer le texte à aller à la ligne
  column_spec(5, width = "5cm") %>% 
  add_header_above(c(" " = 1, "Scores EMD-LD par Phénotype" = 3, " " = 1))
...
library(knitr)
library(kableExtra)
# --- Préparation des données (Comparaison asymétrique des espaces de support : k=50 vs k=5000) ---
diag_matrix_diff <- data.frame(
  Comparaison = c("FarmCPU (Top 50) vs MLM (Top 5000)", 
                  "FarmCPU (Top 50) vs RF (Top 5000)", 
                  "MLM (Top 5000) vs RF (Top 5000)"),
  Angle_Insertion = c("0.53", "0.52", "0.17"),
  Interpretation = c(
    "Orthogonalité élevée des signaux ",
    "Orthogonalité élevée des signaux ",
    "Consensus  "
  )
)

# --- Rendu Kable (alignement strict pour export PDF/LaTeX) ---
# --- Rendu Kable optimisé pour la taille ---
kable(
  diag_matrix_diff,
  caption = "Analyse de divergence par EMD-LD pour le phénotype Angle d'insertion des branches",
  align = "lcl",
  booktabs = TRUE,
  col.names = c("Comparaison des méthodes", "EMD-LD (Angle)", "Interprétation")
) %>%
  kable_styling(
    latex_options = c("striped", "hold_position", "scale_down"), # Auto-ajustement
    full_width = FALSE
  ) %>%
  row_spec(0, bold = TRUE) %>%
  column_spec(1, width = "6cm") # Évite que la colonne Comparaison soit trop étroite
