...
```{r}
Epi_FILE <- "data/Epigenetic_CG.rda"
...
```{r}
epi<- as.data.table(Epigenetic_CG)
epi[, ID := rownames(Genomic)]
...
## ACP pheno
```{r}
pheno <- merge(pheno, loc_df[,list(Population,ID)], by = "ID")
pheno[,2:25]
```

```{r}
pca.pheno <- PCA(pheno[,2:25], quali.sup=24 )
```

```{r}
plotellipses(pca.pheno,24)
```
...
```{r}
K <- K[, ID := rownames(Genomic)]

K <- merge(K, loc_df[,list(Population,ID)], by = "ID")
...
```{r}
pca.K <- PCA(K[,2:201], quali.sup=200 )
```
```{r}
plotellipses(pca.K,200)
```

## MFA Kinship x Phenotype 

```{r}
K_pheno <- merge(K, pheno, by = "ID")
```


```{r}
mfa <- MFA(K_pheno[,2:224], group = c(199,1,23), type = c("s","n","s"), num.group.sup=2,name.group=c("kinship","Population","pheno") )
```
```{r}
plot(mfa,choix="ind",habillage = "Population.x")
barplot(mfa$eig[1:10,1],main="Eigenvalues")
```

# ACP Geno 

```{r}
geno <- merge(geno, loc_df[,list(Population,ID)], by = "ID")
```

```{r}
pca.geno <- PCA(geno[,2:217002], quali.sup = 217001 )
```

```{r}
plot(pca.geno,choix="ind",habillage = "Population")
```
## geno transposed

```{r}
t_geno <- transpose(geno[, 2:217001])
t_geno[, SNP := names(geno)[2:217001]]
setcolorder(t_geno, "SNP")
```
...
```{r}
pca.tgeno <- PCA(t_geno[,2:200])
```



Et bim ! Chromosome 13 !! 
...
dt1 <- all_scores_dt[phenotype == pheno1 & method == method1 & context == context1,
            .(SNP,mean = mean, score = score_scaled, rank = rank)]
```
...
```{r}
snps_sel <- dt1[order(rank)][1:min(100, .N), SNP]



coords_dt <- as.data.table(pca.tgeno$ind$coord[, 1:2])

setnames(coords_dt, c("PC1", "PC2"))

coords_dt[, SNP := names(geno)[2:217001]]

coords_dt[, highlight := ifelse(SNP %in% snps_sel, "selected", "other")]

ggplot(coords_dt, aes(x = PC1, y = PC2)) +
  geom_point(aes(color = highlight),
             alpha = 0.6,
             size = 1) +
  geom_point(
    data = coords_dt[highlight == "selected"],
    color = "red",
    size = 2
  ) +
  geom_text(
  ...
  scale_color_manual(
    values = c("other" = "grey70", "selected" = "red"),
    guide = "none"
  ) +
  theme_minimal() +
  labs(
    title = "ACP sur les SNP (matrice Individus × SNP)",
    x = "PC1",
    y = "PC2"
  )
