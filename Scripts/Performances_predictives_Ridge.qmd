...
all_scores_dt <- read_parquet("results/all_scores.parquet", as_data_table = TRUE, col_select = cols_to_read)

# colonne 'median' comme score global pour le ranking
all_scores_dt[, rank := frank(-median, ties.method = "min"), by = .(phenotype, method, context)]
...
```{r}
## Paramètres
phenos_run   <- c("CIRC2009","CIRC2011","HT2009","BudFlushSlope")
methods_run  <- c("RF")                        # méthodes à évaluer
