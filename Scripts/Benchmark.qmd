...
Pheno_FILE <- "data/LMM_Residuals_Kinship_PC_Pop.rds"
...
#target_pheno <- c("pheno_null")
#target_pheno <- c("CIRC2009")
...
```{r}
#saveRDS(results, file = "results/benchmarks/pheno_null_LMM_residuals_PC_pop.rds")
#saveRDS(results, file = "results/benchmarks/CIRC2009_LMM_residuals_PC_pop.rds")
#saveRDS(results, file = "results/benchmarks/BrAnglVert_LMM_residuals_PC_pop.rds")
```


```{r}

F_circ_res <- c("results/benchmarks/CIRC2009_LMM_residuals_PC_pop.rds")
Circ_res <- rbindlist(lapply(F_circ_res, readRDS), fill = TRUE)

F_brAngl_res <- c("results/benchmarks/BrAnglVert_LMM_residuals_PC_pop.rds")
brAngl_res <- rbindlist(lapply(F_brAngl_res, readRDS), fill = TRUE)

F_null_res <- c("results/benchmarks/pheno_null_LMM_residuals_PC_pop.rds")
null_res <- rbindlist(lapply(F_null_res, readRDS), fill = TRUE)


F_circ <- c("results/benchmarks/CIRC2009_pheno_brut.rds")
Circ <- rbindlist(lapply(F_circ, readRDS), fill = TRUE)

F_brAngl <- c("results/benchmarks/BrAnglVert_pheno_brut.rds")
brAngl <- rbindlist(lapply(F_brAngl, readRDS), fill = TRUE)

F_null <- c("results/benchmarks/pheno_null_brut.rds")
null <- rbindlist(lapply(F_null, readRDS), fill = TRUE)
```
...
plot_data[, method_clean := fcase(
  method == "Random", "Procédure de sélection :\naléatoire",
  #method == "RF" & context == "phenotype", "Procédure de sélection :\nRF - phénotype observé",
  method == "RF" & context == "LMM_residuals", "Procédure de sélection :\nRF - sur résidus modèle mixte",
  method == "MLM", "Procédure de sélection :\nMLM",
  #method == "FarmCPU", "Procédure de sélection :\nFarmCPU",
  default = NA_character_ 
)]
...
ggplot(plot_data, aes(x = topK_factor, y = R2, fill = label_run)) +
  ...
  scale_fill_manual(values = c(
    "Procédure de sélection :\naléatoire" = "#999999",
    "Procédure de sélection :\nRF - phénotype observé" = "#E41A1C",
    "Procédure de sélection :\nRF - sur résidus modèle mixte" = "#FF7F00",
    "Procédure de sélection :\nMLM" = "#377EB8",
    "Procédure de sélection :\nFarmCPU" = "#4DAF4A"
  )) +
  labs(
    title = "Ridge sur phénotype nul (tiré dans une loi normale)",
    x = "Nombre de marqueurs (Top K)",
    y = expression(bold(R^2) ~ "(validation croisée)")
  ) +
  theme(
...
# Comparaison Ridge RF

```{r}
final_dt <- readRDS("~/Code/ACO-CODE_phenotypique /PoplarOmicsORL/Scripts/results/benchmarks/full_res_top100.rds")
...
```{r}
unique(final_dt$context)
unique(final_dt$method)
unique(final_dt$phenotype)
```
...
df_plot <- final_dt %>%
  # Création d'une étiquette propre pour l'axe X (Méthode x Contexte)
  filter(context %in% c("LMM_residuals","GWAS")) %>%
  filter(method %in% c("RF","FarmCPU","MLM")) %>%
  filter(phenotype %in% c("BudFlushSlope","HT2009","Dia2015_sqrt","BrAnglVert","CIRC2011","CIRC2009","RamifSyllep","Rust","pheno_null" )) %>%
  mutate(method_context = paste(method, context, sep = " - ")) %>%
  # Optionnel : Renommer proprement ici si besoin
  # mutate(method_context = fct_recode(method_context, "Nom Propre 1" = "Old_Method - Old_Context")) %>%
  
  # Forcer l'ordre des modèles (Ridge en haut, RF en bas)
  mutate(model = factor(model, levels = c("ridge", "RF")))

# 2. Création du graphique
ggplot(df_plot, aes(x = method_context, y = R2, fill = method_context)) +
  geom_boxplot(alpha = 0.7, outlier.size = 0.5) +
  scale_y_continuous(limits = c(0, 1))+
  
  # Facettage : lignes = model, colonnes = phenotype
  facet_grid(model ~ phenotype) +
  
  # Ajout des médianes au-dessus des boxplots
  stat_summary(
    fun = median, 
    geom = "text", 
    aes(label = sprintf("%.2f", ..y..)),
    vjust = -3.5, # Ajusté pour être juste au dessus
    size = 2.5, 
    fontface = "bold"
  ) +
  ...
  theme_bw() +
  labs(
    x = "Procédure de sélection",
    y = expression(R^2 ~ "de prédiction (CV)"),
    fill = "Configuration"
  ) +
  theme(
  ...
  # Ajuster les limites de l'axe Y pour laisser de la place au texte des médianes
  expand_limits(y = max(df_plot$R2) * 1.1)
```



# phénotype observé vs phénotype déflaté 
...
iles_list <- list(
  # Phénotypes déflatés (Résidus)
  list(p = "results/benchmarks/CIRC2009_LMM_residuals_PC_pop.rds", t = "Circ.", c = "Déflaté"),
  list(p = "results/benchmarks/BrAnglVert_LMM_residuals_PC_pop.rds", t = "Angle", c = "Déflaté"),
  list(p = "results/benchmarks/pheno_null_LMM_residuals_PC_pop.rds", t = "Null", c = "Déflaté"),
  # Phénotypes bruts
  list(p = "results/benchmarks/CIRC2009_pheno_brut.rds", t = "Circ.", c = "Observé"),
  list(p = "results/benchmarks/BrAnglVert_pheno_brut.rds", t = "Angle", c = "Observé"),
  list(p = "results/benchmarks/pheno_null_brut.rds", t = "Null", c = "Observé")
)

plot_data[, method_clean := fcase(
  # On split le Random selon le contexte pour éviter le mélange des 3600 runs
  method %like% "Random" & context == "Déflaté", "Aléatoire (Déflaté)",
  method %like% "Random" & context == "Observé", "Aléatoire (Observé)",
  
  method == "MLM" & context == "Déflaté",        "MLM (Déflaté)",
  method == "MLM" & context == "Observé",        "MLM (Observé)",
  
  method == "FarmCPU" & context == "Déflaté",    "FarmCPU (Déflaté)",
  method == "FarmCPU" & context == "Observé",    "FarmCPU (Observé)",
  
  default = NA_character_
)]

# --- 2. Filtrage et Top K ---
K_selection <- c(50, 100, 500, 1000, 5000)
plot_final <- plot_data[!is.na(method_clean) & topK %in% K_selection]

trait_selection <- c("Circ.", "Angle")
plot_final <- plot_final[ trait %in% trait_selection]


method_selection <- c("Random",  "FarmCPU" ,"MLM"  )
plot_final <- plot_final[ method %in% method_selection]

# --- 3. Gestion des facteurs pour l'ordre d'affichage ---
methods_order <- c(
  "Aléatoire (Observé)", "Aléatoire (Déflaté)",
  "MLM (Observé)",       "MLM (Déflaté)",
  "FarmCPU (Observé)",   "FarmCPU (Déflaté)"
)

plot_final[, method_clean := factor(method_clean, levels = methods_order)]
plot_final[, topK := factor(topK, levels = K_selection)]

```

```{r}
unique(plot_final$method)
```
...
```{r}
library(dplyr)
# 1. Préparation du dataset pour le plot miroir
df_mirror <- plot_final %>%
  # On peut filtrer sur un Top K spécifique pour plus de clarté (ex: 5000 ou 50)
  filter(topK == 50) %>% 
  
  # Organisation des lignes (Contextes) et colonnes (Traits)
  mutate(
    context = factor(context, levels = c("Observé", "Déflaté")),
    trait = factor(trait, levels = c("Angle", "Circ.", "Null")) # Ajuste selon tes noms
  )

# 2. Création du graphique en miroir
ggplot(df_mirror, aes(x = method, y = R2, fill = method)) +
  # Boxplot épuré
  geom_boxplot(alpha = 0.7, outlier.size = 0.5, width = 0.6) +
  
  # Le coeur de la demande : Lignes = Contexte, Colonnes = Phénotype
  facet_grid(context ~ trait) +
  ...
  theme_bw() +
  labs(
    title = "Impact de la déflation de la structure de population (top 5000 SNPs de chaque procédure)",
    subtitle = "Comparaison des R2 pour les prédictions des phénotypes observés (haut) vs résidus (bas)",
    x = "Procédure de sélection",
    y = expression(bold(R^2) ~ "de prédiction (CV)"),
    fill = "Méthode"
  ) +
  theme(
