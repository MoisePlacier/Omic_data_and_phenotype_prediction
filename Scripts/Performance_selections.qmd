...
```{r}
final_dt <- readRDS("~/Code/ACO-CODE_phenotypique /PoplarOmicsORL/Scripts/results/benchmarks/full_res_top5000.rds")
...
med_final <- plot_final_dt[!is.na(model_label), .(R2_median = median(R2, na.rm = TRUE)), 
                           by = .(phenotype, model_label)]

ggplot(plot_final_dt[!is.na(model_label)], aes(x = model, y = R2, fill = model)) +
  ...
  geom_text(
    data = med_final,
    aes(x = model, y = R2_median, label = sprintf("%.2f", R2_median)),
