...
files <- list.files("results/results_random_SNP_PLS", pattern = "\\.rds$", full.names = TRUE)
...
# Création du plot
p <- ggplot(results, aes(x = factor(subset_size), y = R2, fill = factor(subset_size))) +
  geom_boxplot() +
  theme_minimal() +
  labs(
    title = "R² selon la taille du sous-ensemble de SNP",
    x = "Nombre de SNP",
    y = "R² sur l'échantillon test"
  ) +
...
# Sauvegarde
ggsave(
  filename = "Plots/Performance_PLS_Subset_Size.png",
  plot = p,
  width = 8,
  height = 6,
  dpi = 300
