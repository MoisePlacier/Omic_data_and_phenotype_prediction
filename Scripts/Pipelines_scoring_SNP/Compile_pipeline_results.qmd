...
DIRS <- list(
    RF = list(
    root    = "results/RF_all_pheno_all_snp",
    method  = "RF",
    context = "phenotype_all_SNP",
    score_col = "importance"
  )
)
...
final_file <- "results/RF_scores_all_pheno_all_snp.parquet"
...
```{r}
dt1 <- read_parquet("results/gwas_results_pheno_null.parquet", as_data_table = TRUE)
```
```{r}
dt1[,p_value := NULL]
dt1
