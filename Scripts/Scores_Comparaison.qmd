...
```{r}
phenos_run <- c("BudFlushSlope","HT2009","Dia2015_sqrt","BrAnglVert","CIRC2011","CIRC2009","RamifSyllep","Rust","pheno_null"
)
topK_run = 500


RF_res_res <- compare_all_phenos_fast(
  all_scores_dt, phenos_run, "RF", "LMM_residuals", topK = topK_run)
...
RF_res_res[, context := "RF_LMM_residuals"]
farmcpu_run[, context := "FarmCPU"]
mlm_run[, context := "MLM"]


# Fusionner les deux résultats pour comparer
res_all <- rbind( RF_res_res,farmcpu_run,mlm_run)
...
ggplot(res_all, aes(x = pheno1, y = pheno2, fill = topK_overlap)) +
  geom_tile() +
  # Ajout conditionnel des labels
  geom_text(aes(label = ifelse(topK_overlap > 0.01, sprintf("%.2f", topK_overlap), "")), 
            color = "black", 
            size = 3, 
            fontface = "bold") +
  facet_wrap(~ context) +
  scale_fill_gradient2(low = "blue", mid = "white", high = "red", midpoint = 0, limit = c(0,1)) +
  theme_minimal() +
  labs(
  ...
  theme(
    ...
    axis.text.y = element_text( hjust = 1, size = 11, face = "bold"),
...
plot_overlap <- function(overlap_df){
  ...
  labs(title = paste("pourcentage overlap pour le Top", 500, "SNP\n Circonférence 2009")) +
  theme_minimal() +
...
dt_plot <- all_scores_dt[
  method_context %in% c("RF_LMM_residuals","FarmCPU_GWAS","MLM_GWAS") &
  #& chrom %in% c("Chr13") 
  #rank <200 &
  phenotype %in% c("CIRC2009","BrAnglVert","pheno_null")
]
...
ari_bp <- compute_multi_ari(
  all_scores_dt, "CIRC2009", pairs_to_compare,
...
# Wasserstein sur distance physique
...
global_results <- rbindlist(lapply(pairs, function(p) {
  val <- compute_global_linear_emd(all_scores_dt, "BrAnglVert", p[1], p[2], score_col = "mean")
...
null_pd_emd_500KB <- rbindlist(lapply(pairs, function(p) {
  dt_res <- compute_emd_by_chromosome(
    dt = all_scores_dt, 
    phenotype_target = "pheno_null", 
    method1 = p[1], 
    method2 = p[2],
    score_col = "mean"
  )
  # Création du label de comparaison
  dt_res[, comparaison := paste(p[1], "vs", p[2])]
  return(dt_res)
}))
```
...
```{r}
plot_emd_barplot_chr(BR_pd_emd_500KB, title_label = "0,5 Mb - BrAnglVert")
plot_emd_barplot_chr(circ_pd_emd_500KB, title_label = "0,5 Mb - Circonférence 2009")
plot_emd_barplot_chr(null_pd_emd_500KB, title_label = "0,5 Mb - phénotype aléatoire")
```
...
```{r}
variances <- apply(geno_dt, 2, var, na.rm = TRUE)

cols_to_keep <- which(variances > 0 & !is.na(variances))

geno_dt_clean <- geno_dt[, ..cols_to_keep]
...
results_ld_null <- compute_multi_ld_emd(
  geno_mat = X_full,
...
# --- EXÉCUTION ---
my_pairs_diff <- list(
  list(m1 = "FarmCPU_GWAS",      k1 = 50,   m2 = "RF_LMM_residuals", k2 = 1000),
  list(m1 = "FarmCPU_GWAS",      k1 = 50,   m2 = "MLM_GWAS",         k2 = 1000),
  list(m1 = "MLM_GWAS",          k1 = 1000, m2 = "RF_LMM_residuals", k2 = 1000)
)

results_diff <- compute_multi_ld_emd_diffK(
  geno_mat = X_full, 
  scores_dt = all_scores_dt, 
  phenotype_target = "BrAnglVert", 
  pairs_list = my_pairs_diff,
  score_col = "mean"
)
...
ld_emd_brangl <- compute_multi_ld_emd_by_chr(
    geno_mat = X_full,           
    scores_dt = all_scores_dt,          
    phenotype_target = "BrAnglVert",   
    pairs_list = my_pairs,         
...
```
```{r}
plot_ld_emd_barplot_chr(ld_emd_brangl, title_label = "- Angle insertion des branches")
plot_ld_emd_barplot_chr(ld_emd_circ, title_label = "- Circonférence 2009")
plot_ld_emd_barplot_chr(ld_emd_null, title_label = "- Phénotype aléatoire")
```
```{r}
my_pairs_diff <- list(
  list(m1 = "FarmCPU_GWAS",     k1 = 50,   m2 = "RF_LMM_residuals", k2 = 1000),
...
my_pairs_equal <- list(
  list(m1 = "FarmCPU_GWAS",     k1 = 1000,   m2 = "RF_LMM_residuals", k2 = 1000),
  list(m1 = "FarmCPU_GWAS",     k1 = 1000,   m2 = "MLM_GWAS",         k2 = 1000),
  list(m1 = "MLM_GWAS",         k1 = 1000, m2 = "RF_LMM_residuals", k2 = 1000)
)

# Lancement
null_emd <- compute_multi_ld_emd_by_chr_kdependant(
    geno_mat = X_full,
    scores_dt = all_scores_dt,
    phenotype_target = "pheno_null",
    pairs_list = my_pairs_equal
)

```
```{r}
plot_ld_emd_barplot_chr(br_k_emd, title_label = "- angle insertion branches Comparaison Top K Différenciés")
plot_ld_emd_barplot_chr(br_emd, title_label = "- angle insertion branches")

plot_ld_emd_barplot_chr(cr_emd, title_label = "- angle insertion branches Comparaison Top K Différenciés")
plot_ld_emd_barplot_chr(null_emd, title_label = "- angle insertion branches")
```

```{r}
my_pairs_diff <- list(
  list(m1 = "FarmCPU_GWAS",     k1 = 50,   m2 = "RF_LMM_residuals", k2 = 5000),
  list(m1 = "FarmCPU_GWAS",     k1 = 50,   m2 = "MLM_GWAS",         k2 = 5000),
  list(m1 = "MLM_GWAS",         k1 = 5000, m2 = "RF_LMM_residuals", k2 = 5000)
)

my_pairs_equal <- list(
  list(m1 = "FarmCPU_GWAS",     k1 = 5000,   m2 = "RF_LMM_residuals", k2 = 5000),
  list(m1 = "FarmCPU_GWAS",     k1 = 5000,   m2 = "MLM_GWAS",         k2 = 5000),
  list(m1 = "MLM_GWAS",         k1 = 5000, m2 = "RF_LMM_residuals", k2 = 5000)
)

# Lancement
br_k_emd_5000 <- compute_multi_ld_emd_by_chr_kdependant(
    geno_mat = X_full,
    scores_dt = all_scores_dt,
    phenotype_target = "BrAnglVert",
    pairs_list = my_pairs_diff
...
```{r}
plot_ld_emd_barplot_chr(br_k_emd_5000, title_label = "- angle insertion branches Comparaison Top K Différenciés")
plot_ld_emd_barplot_chr(br_emd_5000, title_label = "- angle insertion branches")

plot_ld_emd_barplot_chr(cr_emd_5000, title_label = "- Circonférence 2009")
plot_ld_emd_barplot_chr(null_emd_5000, title_label = "- phenotype aléatoire")
```


## EMD ld window
```{r}
all_scores_dt <- all_scores_dt[!is.na(mean)]
```


```{r}
circ_5KB_LDEMD <- compute_emd_ld_by_window(
  geno_mat = X_full,
  dt = all_scores_dt,
  phenotype_target = "CIRC2009",
  pairs_list = my_pairs,
  window_bp = 5e5 # Fenêtres de 1 Mb
)
```

```{r}
plot_emd_ld_window_profile(Br_5KB_LDEMD, window_label = "0,5 Mb - Angle insertion branches")
plot_emd_ld_window_profile(null_5KB_LDEMD, window_label = "0,5 Mb - phénotype aléatoire")
plot_emd_ld_window_profile(circ_5KB_LDEMD, window_label = "0,5 Mb - Circonférence 2009")
```
