...
for(ph in phenotypes){
  ...
  p <- ggplot(tmp_dt, aes(x = score_scaled, y = method_context, fill = method_context)) +
...
phenos_run <- c("CIRC2009","CIRC2011","HT2009","BudFlushSlope","BrAnglVert","Dia2015_sqrt","RamifSyllep","Rust","pheno_pop_aware","pheno_kin_aware","pheno_null")
method_run <- "RF"
...
phenos_run <- c("CIRC2009","CIRC2011","HT2009","BudFlushSlope","BrAnglVert","Dia2015_sqrt","RamifSyllep","Rust")
method_run <- "FarmCPU"
context_run <- "GWAS"
topK_run <- 100

farmcpu_run <- compare_all_phenos_full_safe(all_scores_dt, phenos_run, method_run, context_run, topK_run)
...
mlm_run <- compare_all_phenos_full_safe(all_scores_dt, phenos_run, method_run, context_run, topK_run)
...
```{r}
all_scores_dt[method_context %in% c("RF_residuals","RF_phenotype","VIP_residuals","FarmCPU_GWAS","MLM_GWAS") &
                  chrom %in% c("Chr10") & score_scaled >= 0.1,
                  phenotype %in% c("BrAnglVert","RamifSyllep")]
```
...
dt_plot <- all_scores_dt[
  ...
  chrom %in% c("Chr13") &
  rank <200 &
  phenotype %in% c("HT2009","CIRC2011","CIRC2009")
]
