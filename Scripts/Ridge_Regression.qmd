...
all_dt_loaded[, rank := frank(-median, ties.method = "min"), by = .(phenotype, method, context)]

# normaliser le ranking sur 0-1 
all_dt_loaded[, rank_norm := rank / max(rank), by = .(phenotype, method, context)]
...
## Selection d'un phéno x methode x contexte

```{r}
pheno_sel <- "Dia2015_sqrt"
context <- "phenotype"
method <- "VIP"

dt <- all_dt_loaded[method == method & phenotype == pheno_sel & context == context,
                      .(SNP, mean = mean, rank = rank)]
dt
```
...
```{r}
K<- 2000
top_K  <- dt[order(-mean)][1:K, SNP]
length(top_K)

X_sub <- X[, top_K, drop = FALSE]
dim(X_sub)
```
